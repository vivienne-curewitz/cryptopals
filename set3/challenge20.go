...
const input20 string = `SSdtIHJhdGVkICJSIi4uLnRoaXMgaXMgYSB3YXJuaW5nLCB5YSBiZXR0ZXIgdm9pZCAvIFBvZXRzIGFyZSBwYXJhbm9pZCwgREoncyBELXN0cm95ZWQ=
...
func c20() {
	key := getKey()
	ciphertexts := generateMany(key, input20)
	keysize := getSmallest(ciphertexts)
	var ciphers_cat []byte
	for _, cipher := range ciphertexts {
		ciphers_cat = append(ciphers_cat, cipher[:keysize]...)
	}
	transpose := xor.CipherTranspose(ciphers_cat, keysize)
	likelyKeys := xor.SolveBlocks(transpose)
	// log.Printf("Likely Keys: %s\n", likelyKeys)
	// decrypt
	for _, c := range ciphertexts {
		decrypted := xor.XorBytes(c[:keysize], likelyKeys)
		log.Printf("%s\n", string(decrypted))
	}
}
